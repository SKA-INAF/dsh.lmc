...
def init_dev_from_xml_config(db,tree,clear_servers):
	...
	for cs_node in tree.iter('ControlSystem'):
		...
		for server_node in servers_node.iter('server'):
			...
			## NB: If astor controlled attr is not given, assume it is not controlled by Astor
			## NB: If controlled=false, ignore startup level.
			## NB: If controlled=true and level missing assume startup level=1
			astor_controlled= 0
			startup_level= 0
			if 'astor_controlled' in server_node.attrib :
				astor_controlled_flag= server_node.attrib.get('astor_controlled')
				if astor_controlled_flag=='true':
					astor_controlled= 1
			...
			for class_node in server_node.iter('class'):
				...
				for device_node in class_node.iter('device'):
					...
					if device_configuration_node is None :
						...
						continue # Go to next device (if no config file has to be parsed), rather than throwing exception
						#raise Exception(errMsg)
			...
			serv_info.mode= int(astor_controlled)
...
def main():
	...
	module = AnsibleModule(
		argument_spec=dict(
			...
			clear_servers=dict(required=False, type='bool', default=False),
			export_servers=dict(required=False, type='bool', default=False),
		),
