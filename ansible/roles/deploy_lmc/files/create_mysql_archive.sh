...
if [ $# -ne "$ARGS" ]; then 
	echo "ERROR: Invalid number of arguments (passed $# parameters)!" 
  exit 
fi 
...
COMMAND='mysql --batch --user='"'""$mysql_user""'"' --password='"'""$mysql_pass""'"' --database='"'""$archive_db_name""'"
echo $COMMAND


##mysql –u $mysql_user -p $mysql_pass -A -e "set @archive_db_name=${archive_db_name}; source {sql_script};" #>${data_file};
#mysql -u $mysql_user -p $mysql_pass -A -e "set @archive_db_name=${archive_db_name}; source {sql_script};" 
  
#mysql --batch --user="'""$mysql_user""'" --password="'""$mysql_pass""'" --database="'""$archive_db_name""'" << "EOF"
eval $COMMAND << "EOF"
