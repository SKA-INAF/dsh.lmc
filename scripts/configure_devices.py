...
#		self.schemafile= ""
#		self.loglevel= "INFO"
#		self.clear_servers= False
#		self.export_servers= False
...
def get_args():
	...
	parser.add_argument('-e', '--export', dest='export_servers', required=False, action='store_true', default=False, help='Export registered servers')
	...
	#export_servers= args.export_servers
	
	#options= ProgramOptions()
	#options.inputfile= args.inputfile
	#options.schemafile= args.schemafile
	#options.loglevel= args.loglevel
	#options.clear_servers= args.clear_servers

	#return inputfile, schemafile, loglevel, export_servers
	return args
...
def main():
	...
	## Get script args
	#filename, filename_schema, loglevel, clear_servers, export_servers = get_args()
	try:
		args= get_args()
	except Exception as ex:
	...
	filename_schema= args.schemafile
	loglevel= args.loglevel
	clear_servers= args.clear_servers
	start_servers= args.export_servers	
	...
	try: 	
		...
		logging.error("Tango device initialization failed with err= %s",str(ex))
		return 1

	## Export registered servers
	## NB: This will startup servers and then kill it after reaching a given State
	if start_servers:
		logging.info("Trying to startup and export registered servers...")
		try:
			export_servers(db,registered_servers)
		except Exception as ex:
			logging.error("Failed to export registered servers (err= %s)",str(ex))
			return 1
