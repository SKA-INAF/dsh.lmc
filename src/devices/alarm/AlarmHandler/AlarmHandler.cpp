...
void AlarmHandler::delete_device()
{
	...
	_DEBUG_LOG(__func__ << " starting="<<(int)starting << " shutting_down="<<(int)shutting_down<<" restarting="<<(int)restarting);

	abortflag = true;
	_DEBUG_LOG("after abortflag=true ...");
	try {
	...
	_DEBUG_LOG("events unsubscribed!");
...
void AlarmHandler::init_device()
{
	...
	attr_alarm_read = new Tango::DevString[1024];
