		...
		string formula, unsigned int on_delay, unsigned int off_delay, string grp, string lev, string msg, string cmd_a, string cmd_n, int silent_time, vector<string> alm_list)
{
	...
	dbd_att_name << (short int)11;                      //Commit f6544466b27d948952cc383ad93f09a3d095b7ef (25 Aug 2017)
	dbd_name << name;
	dbd_formula << formula;
	//dbd_on_delay << on_delay;
	//dbd_off_delay << off_delay;
	dbd_on_delay << (Tango::DevLong)on_delay;//Commit f6544466b27d948952cc383ad93f09a3d095b7ef (25 Aug 2017)
	dbd_off_delay << (Tango::DevLong)off_delay;//Commit f6544466b27d948952cc383ad93f09a3d095b7ef (25 Aug 2017)
	dbd_level << lev;
	//dbd_silence_time << silent_time;
	dbd_silence_time << (Tango::DevLong)silent_time;//Commit f6544466b27d948952cc383ad93f09a3d095b7ef (25 Aug 2017)
	dbd_group << grp;
	dbd_message << msg;
	dbd_oncommand << cmd_a;
	dbd_offcommand << cmd_n;
	//dbd_enabled << (enabled ? 1 : 0);
	dbd_enabled << (enabled ? (short int)1 : (short int)0);//Commit f6544466b27d948952cc383ad93f09a3d095b7ef (25 Aug 2017)
...
void alarm_table::get_alarm_list_db(vector<string> &al_list, map<string, string> &saved_alarms)
{
	...
	for (size_t i=0;i < db_data.size();/*i++*/)
	{
		...
		for (long k=0;k < nb_prop;k++)
		{
			...
			{
				cout << "att_name="<<att_name<<" UNKNOWN prop_name="<<prop_name<<endl;
				i++;
				continue;
			}
			i++;
