...
void HdbDevice::initialize()
{
	...
	cout<<"HdbDevice::initialize(): before build_signal_vector()"<<endl;
	build_signal_vector(list, defaultStrategy);
	cout<<"HdbDevice::initialize(): after build_signal_vector()"<<endl;
...
void HdbDevice::add(string &signame, vector<string> contexts, Tango::DevULong ttl)
{
	...
	shared->add(signame, contexts, ttl, HDB_UPDATE_PROP, false);
}
//=============================================================================
//=============================================================================
...
void HdbDevice::get_hdb_signal_list(vector<string> & list)
{
	...
	for (unsigned int i=0 ; i<tmplist.size() ; i++)
	{
		...
		INFO_STREAM<<"tmplist[i]="<<tmplist[i];
...
bool HdbDevice::compare_without_domain(string str1, string str2)
{
	...
	std::transform(str1_nd.begin(), str1_nd.end(), str1_nd.begin(), (int(*)(int))tolower);
	std::transform(str2_nd.begin(), str2_nd.end(), str2_nd.begin(), (int(*)(int))tolower);
	//cout<<"compare_without_domain(): str1_nd="<<str1_nd<<", str2_nd="<<str2_nd<<endl;
