...
{
	//====  ADDED BY SIMONE ====
	//std::transform(signame.begin(), signame.end(), signame.begin(), (int(*)(int))tolower);
	//==========================
	...
	for (unsigned int i=0 ; i<signals.size() ; i++)
	{
		...
		//cout<<"SharedData::get_signal() - signame="<<signame<<", signal[i]="<<sig->name<<endl;
	...
	for (unsigned int i=0 ; i<signals.size() ; i++)
	{
		...
		//	cout<<"SharedData::get_signal() - signame="<<signame<<", signal[i]="<<sig->name<<" MULTI_TANGO_HOST comparison? "<<hdb_dev->compare_without_domain(sig->name,signame)<<endl;
		//#else
		//	cout<<"SharedData::get_signal() - signame="<<signame<<", signal[i]="<<sig->name<<" comparison? "<<hdb_dev->compare_tango_names(sig->name,signame)<<endl;
		//#endif
	...
	//INFO_STREAM<<"Returning NULL signal!!!"<<endl;
		...
		for (unsigned int i=0 ; i<signals.size() && !found ; i++, pos++)
		{
			...
			if (sig->name==signame)
			{
				...
				if(stop)
				{
					...
					try
					{
						if(sig->event_id != HDB_ERR)
						{
							delete sig->archive_cb;
						}
						sig->event_id = HDB_ERR;
						if(sig->attr)
							delete sig->attr;
						sig->attr = NULL;
					}
		...
		if (signals[i].name==signame)
		{
			...
			if(!signals[i].running)
			{
				if(signals[i].stopped)
				{
					...
					try
					{
						add(signame, contexts, ttl, HDB_NOTHING, true);
					}
					catch (Tango::DevFailed &e)
					{
...
void SharedData::unsubscribe_events()
{
	...
	for (unsigned int i=0 ; i<local_signals.size() ; i++)
	{
		...
		if (local_signals[i].event_id != HDB_ERR && sig->attr)
		{
	...
	for (unsigned int i=0 ; i<signals.size() ; i++)
	{
		HdbSignal	*sig = &signals[i];
		sig->siglock->writerIn();
		if (signals[i].event_id != HDB_ERR && sig->attr)
		{
			delete sig->archive_cb;
	...
	{
		signals[j].event_id = HDB_ERR;
		signals[j].event_conf_id = HDB_ERR;
...
{
	add(signame, contexts, ttl, HDB_NOTHING, false);
}
//=============================================================================
/**
 * Add a new signal.
...
void SharedData::add(string &signame, const vector<string> & contexts, Tango::DevULong ttl, int to_do, bool start)
{
	DEBUG_STREAM << "SharedData::"<<__func__<<": Adding " << signame << " to_do="<<to_do<<" start="<<(start ? "Y" : "N")<< endl;
	{
		...
		signal->event_id = HDB_ERR;
		signal->event_conf_id = HDB_ERR;
		...
		if (!found && !start)
		{
			veclock.writerIn();
			//	Add in vector
			INFO_STREAM<<"Adding signal "<<signal->name<<" in list..."<<endl;
...
void SharedData::subscribe_events()
{
	...
	for (unsigned int i=0 ; i<signals.size() ; i++)
	{
		HdbSignal	*sig = &signals[i];
		sig->siglock->writerIn();
		if (sig->event_id==HDB_ERR && !sig->stopped)
		{
			if(!sig->attr)
			{
				try
				{
					vector<string> contexts;	//TODO: not used in add(..., true)!!!
					Tango::DevULong ttl;	//TODO: not used in add(..., true)!!!
					add(sig->name, contexts, ttl, HDB_NOTHING, true);
				}
				catch (Tango::DevFailed &e)
				{
					//Tango::Except::print_exception(e);
			...
			sig->siglock->writerOut();
			int		event_id = HDB_ERR;
			int		event_conf_id = HDB_ERR;
			bool	isZMQ = true;
			bool	err = false;
...
int SharedData::nb_sig_to_subscribe()
{
	...
	{
		signals[i].siglock->readerIn();
		if (signals[i].event_id == HDB_ERR && !signals[i].stopped)
		{
			nb++;
		}
		signals[i].siglock->readerOut();
...
void *SubscribeThread::run_undetached(void *ptr)
{
	...
	while(shared->get_if_stop()==false)
	{
		...
		//	wait a long time if all signals subscribed
		{
			...
			//shared->lock();
			if (nb_to_subscribe==0 && shared->action == HDB_NOTHING)
			{
			...
			else if(shared->action == HDB_NOTHING)
			{
