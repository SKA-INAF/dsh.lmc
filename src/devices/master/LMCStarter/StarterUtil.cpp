 ...
 */
//+------------------------------------------------------------------
StarterUtil::StarterUtil(LMCStarter* dev_instance,Tango::DeviceProxy *database, vector<string> host_names, string logHome)
{
	m_dev= dev_instance;
	dbase = database;
	...
	starter_log_file += "/LMCStarter.log";

	LogPath(starter_stat_file, logHome);
	//starter_stat_file += "/Starter.stat";
	starter_stat_file += "/LMCStarter.stat";
...
void StarterUtil::build_server_ctrl_object(vector<ControlledServer> *servers)
{
	...
	for (pos=result.begin() ; pos<result.end() ; )
	{
		...
		if (p_serv==NULL)
		{
			...
			__INFO_LOG(m_dev,"Adding server "<<name<<" (adm_name="<<server.admin_name<<", controlled? "<<server.controlled<<", level="<<server.startup_level<<") to the Starter server list...");
